...
prim_count = chicagoData['month'].value_counts(normalize=True)
prim_count = prim_count[:31,]
rect2 = ax1.bar(prim_count.index, prim_count.values, label='Chicago')
ax1.set_title('Chicago Monthly Crimes')

prim_count2 = sfData['month'].value_counts(normalize=True)
prim_count2 = prim_count2[:31,]
rect2 = ax2.bar(prim_count2.index, prim_count2.values, label='SF')
ax2.set_title('San Fransisco Monthly Crimes')
